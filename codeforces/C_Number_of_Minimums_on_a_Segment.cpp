...
struct SegTree {
    ...
    // - For Range GCD: 0
    // - For Range XOR: 0
    const Node NEUTRAL = {LLONG_MAX,0}; 
    ...
    void build(vector<int>& a, int x, int lx, int rx) {
        if (rx - lx == 1) { // Leaf node (represents a single element)
            if (lx < a.size()) {
                tree[x] = {a[lx],1}; // STEP 3: You might need to change how a single element is stored
            }
            return;
        }
    ...
    void update(int i, int v, int x, int lx, int rx) {
        if (rx - lx == 1) { // Reached the leaf node for index 'i'
            tree[x] = {v,1};    // STEP 4: Change this if the update adds to the existing value (tree[x] += v)
            return;
        }
    ...
    Node query(int l, int r, int x, int lx, int rx) {
        ...
        int m = lx + (rx - lx) / 2;
        Node left_result = query(l, r, 2 * x + 1, lx, m);
        Node right_result = query(l, r, 2 * x + 2, m, rx);
